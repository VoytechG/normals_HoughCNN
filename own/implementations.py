...
def color_overlaps(tm1, tm2, color_1, color_2, color_overlap, metric="median"):
    ...
    def color_overlaps_in_one_mesh(
        to_color_tmesh, reference_tmesh, color_overlap, color_non_overlap
    ):
...
def visualise_overlaps(tm1, tm2, metric="median"):
    ...
    tm1_org = tm1.copy().color_verts(cs["red"]).apply_translation([0.1, 0.12, -0.15])
    tm2_org = tm2.copy().color_verts(cs["blue"]).apply_translation([0.1, -0.12, -0.15])
...
def subsamplePointsRandomly(points, subset_size=1):
    ...
    sampled_indexes = np.random.choice(
        number_of_points, int(number_of_points * subset_size), replace=False
    )
...
def icp(
    ...
    def print_info(i, error, delta):
        print(
            "Iteration: {0} \t error: {1:.4f} \t delta: {2:.4f}".format(
                i + 1, error, delta
            )
        )
    ...
    for i in range(max_iterations):
        ...
        if rejection:
            ...
            PQ_pairs_inds = np.array(
                [
                    [P_ind, Q_ind]
                    for Q_ind, (dist, P_ind) in enumerate(
                        zip(PQ_dists[:, 0], P_corresponding_inds[:, 0])
                    )
                    if dist <= median * 3
                ]
            )
        ...
        A = np.sum(
            [
                ...
                for (p_hat_i, q_hat_i) in zip(P_hat, Q_hat)
            ],
            0,
        )
        ...
        error = np.sum(
            [
                np.linalg.norm(
                    p.reshape(3, 1) - np.matmul(R, q.reshape(3, 1)) - t.reshape(3, 1), 2
                )
                ** 2
                for (p, q) in zip(P_filtered, Q_filtered)
            ]
        )
...
def perform_icp_for_rotated_mesh(tmesh_fixed, angle_yaxis, tree_fixed):
    tmesh_rotated = (
        tmesh_fixed.copy().rotate_in_yaxis(angle_yaxis).color_verts(cs["blue_bright"])
    )
...
def icp_point_to_plane(
    ...
    for i in range(max_iterations):
        ...
        b = np.array(
            [
                -np.dot((q - p), n)
                for (q, p, n) in zip(Q_filtered, P_filtered, P_filtered_normals)
            ]
        )
        ...
        for i, mesh_parent in enumerate(meshes):
            ...
            for j, mesh_child in enumerate(meshes):
                ...
                for i in range(max_iterations):
                    ...
                    R = utv
                    t = p_fil_mean.reshape(3, 1) - np.matmul(
                        R, q_fil_mean.reshape(3, 1)
                    )
