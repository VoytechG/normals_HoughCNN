...
result_directory = "trained_model"
dataset_directory = "datasetk3.p"
...
if create_dataset:
    ...
    for i in tqdm(range(0, dataset_size, batch_size), ncols=80):
        nbr, batch, batch_targets = estimator.generate_training_accum_random_corner(
            batch_size
        )
        ...
        if count >= dataset_size:
            break
        estimator.saveXYZ(f"3dmodels_3ks/model_{i}.xyz")
        print(count)
    # save the dataset
    ...
    pickle.dump(dataset, open(os.path.join(dataset_directory, dataset_directory), "wb"))
    print("-->done")
...
if train:
    ...
    dataset = pickle.load(open(os.path.join(dataset_directory, dataset_directory), "rb"))
