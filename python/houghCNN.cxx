														 ...
														 bool compute_P = true, Matrix3 P_ref = Matrix3())
{
	...
	for (int i = 0; i < nbh_size; i++)
	{
		if (min_prob > proba_vector[nbh[i]])
			min_prob = proba_vector[nbh[i]];
		if (max_prob < proba_vector[nbh[i]])
			max_prob = proba_vector[nbh[i]];
	}
...
void NormEst::get_batch(int batch_id, int batch_size, double *array)
		...
		int noise_val, int n_points, double *array, double *array_gt)
{
	...
	{
		const Vector3 &pt = _pc.row(pt_id); //reference to the current point
		if (pt.squaredNorm() > max_square_dist)
		{
			continue;
		}
